...
export default class BookAStall extends Component {
  ...
  marketChoose = market => {
    const eventsSelected = this.state.events.filter(
      e => e.venueSlug === MARKETS[1],
    );
    this.setState({ marketSelected: market, dateSelected: '', eventsSelected });
    //this.setState({ marketSelected: MARKETS[1], dateSelected: '', eventsSelected });
  };

  gotoTicket = (dateSelected: any = null) => {
    //console.log('this.state.marketSelected-',this.state.marketSelected);
   // console.log('this.state.events-',this.state.events);
    // let tickets = this.state.events.filter(
    //   e => e.venueSlug === this.state.marketSelected,
    // );
    let tickets = this.state.events.filter(
      e => e.venueSlug === 'blacktown-markets',
    );
   ...
   // console.log('beftre send -',tickets)
              ...
              <View style={styles.chooseMarketplace}>
                ...
                <TouchableOpacity
                    ...
                    <Image
                      ...
                      resizeMode={'contain'}
                      source={require('../assets/Blacktown_Midweek_Markets.jpg')}
                    />
                  </TouchableOpacity>
                  {/* <TouchableOpacity
                    onPress={() => this.marketChoose(MARKETS[0])}
